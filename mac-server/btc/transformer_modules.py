"""Transformer building blocks — vendored from BTC-ISMIR19 (utils/transformer_modules.py).

Source: https://github.com/jayg996/BTC-ISMIR19
License: MIT (see upstream repo).

Modified: np.float → np.float64 (numpy>=1.20 deprecation).
"""
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        if self.bias_mask is not None:
            logits += self.bias_mask[:, :, : logits.shape[-2], : logits.shape[-1]].type_as(
                logits.data
            )
